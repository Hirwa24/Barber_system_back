...
const DEFAULT_MANAGER_PASSWORD =
  process.env.DEFAULT_MANAGER_PASSWORD || "bsm123";
...
async function enrichManagers(managers) {
  ...
  const barberCountMap = new Map(
    barberCounts.map((item) => [String(item._id), item.count]),
  );
  const walletMap = new Map(
    walletTotals.map((item) => [String(item._id), item.walletBalance]),
  );
  ...
  return managers.map((manager) => ({
    ...
    isActive: Boolean(
      manager.lastLoginAt && manager.lastLoginAt >= activeCutoff,
    ),
  }));
}
...
router.post(
  ...
  [
    ...
    body("phone")
      .matches(/^(\+250|250|0)7\d{8}$/)
      .withMessage("Valid Rwanda phone is required"),
    body("salonName")
      .trim()
      .isLength({ min: 2 })
  ...
  async (req, res) => {
    ...
    try {
      ...
      const defaultPass =
        req.manager?.defaultPassword || DEFAULT_MANAGER_PASSWORD;
...
router.get("/users", [auth, admin], async (req, res) => {
  try {
    const users = await Manager.find()
      .select("-passwordHash -defaultPassword")
      .sort({ createdAt: -1 });
...
router.get("/overview", [auth, admin], async (req, res) => {
  try {
    const managers = await Manager.find({ role: "manager" }).select(
      "-passwordHash",
    );
    ...
    const overview = {
      ...
      activeManagers: enrichedManagers.filter((manager) => manager.isActive)
        .length,
      totalBarbers: enrichedManagers.reduce(
        (sum, manager) => sum + manager.barberCount,
        0,
      ),
      totalWalletBalance: enrichedManagers.reduce(
        (sum, manager) => sum + manager.walletBalance,
        0,
      ),
    };
...
router.patch("/users/:id/role", [auth, admin], async (req, res) => {
  try {
    ...
    const user = await Manager.findByIdAndUpdate(req.params.id, updates, {
      new: true,
    }).select("-passwordHash");
    res.json(user);
  } catch (err) {
