...
router.post('/', auth, [ body('name').isLength({ min: 2 }) ], async (req, res) => {
	const errors = validationResult(req);
	if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
	const { name, phone } = req.body;
	const barber = await Barber.create({ manager: req.managerId, name, phone });
	res.status(201).json(barber);
});

router.put('/:id', auth, async (req, res) => {
	const { name, phone, active } = req.body;
	const barber = await Barber.findOneAndUpdate({ _id: req.params.id, manager: req.managerId }, { name, phone, active }, { new: true });
